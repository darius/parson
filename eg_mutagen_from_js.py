...
""")(hug = hug,
     ...
     mk_sequence = lambda *xs: ' '.join('(%s)' % x for x in xs),
     mk_shuffle  = lambda *xs: '{ %s }' % (' / '.join(xs)),
     mk_weighted = lambda *spairs: ' / '.join('[%s] %s' % pair for pair in zip(spairs[0::2], spairs[1::2])),
...
## goreyfate = open('goreyfate.js').read()
## translate(goreyfate)
